"""Cruza productos SMK con la venta SMK Raw más reciente por artículo."""
...
PRODUCTS = Path(__file__).resolve().parent / "productos_smk_2026.xlsx"
SALES = Path(__file__).resolve().parent / "ventas_smk_raw_2026.xlsx"
OUTPUT = Path(__file__).resolve().parent / "comparacion_smk_2026.xlsx"
...
def main() -> None:
    ...
    titles = {"Marca distinta": 0, "Origen distinto": 0}
    sheets = {}
    for title in titles:
        sheet = workbook.add_worksheet(title)
        sheet.hide_gridlines(2); sheet.freeze_panes(1, 0)
        sheet.write_row(0, 0, headers, header)
        sheet.set_column("A:B", 18, text); sheet.set_column("C:C", 24, text)
        sheet.set_column("D:I", 24, text)
        sheets[title] = sheet
    ...
    for article, (_, sale) in latest_by_article.items():
        for product in products_by_sku.get(article, []):
            comparisons = [
                ...
                ("Origen distinto", product.get("origen") or "", sale.get("categoria_visualizacion") or "", origin_key),
            ]
            for title, product_value, sale_value, normalize in comparisons:
                ...
                titles[title] += 1
    workbook.close()
    print(f"Archivo: {OUTPUT.resolve()} | Marca: {titles['Marca distinta']} | Origen: {titles['Origen distinto']} | Artículos con venta: {len(latest_by_article)}", flush=True)
